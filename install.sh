...
cp ./start-trake-onboot.sh /usr/local/bin
chmod 744 /usr/local/bin/start-trake-onboot.sh
(crontab -l ; echo "@reboot /usr/local/bin/start-trake-onboot.sh") 2>&1 | grep -v "no crontab" | sort | uniq | crontab -
cd src
